...
class VariantSelects extends HTMLElement {
  ...
  updateMasterId() {
    ...
    this.currentVariant = this.getVariantData().find((variant) => {
      const matchingOptions = this.options.every((selectedOption) => {
  ...
  setInputAvailability(listOfOptions, listOfAvailableOptions) {
    console.log('listOfOptions:', listOfOptions); // Log the value
    console.log('listOfAvailableOptions:', listOfAvailableOptions); // Log the value
...
class VariantRadios extends VariantSelects {
  ...
  updateOptions() {
    const selectInputs = Array.from(this.querySelectorAll('select'));
    const radioInputs = Array.from(this.querySelectorAll('input[type="radio"]:checked'));

    this.options = [...selectInputs, ...radioInputs].map(input => input.value);
...
class VariantSelector extends HTMLElement {
  ...
  getSelectedVariant() {
    this.currentVariant = this.getVariantJSON().find((variant) => {
      const matchingOptions = variant.options.filter((option, index) => {
        return this.options[index] === option;
      });

      return matchingOptions.length === this.options.length;
    });

    // If no variant is found and only one option is selected, try to find the variant using only that option
    if (!this.currentVariant && this.options.length === 1) {
      const singleOption = this.options[0];
      this.currentVariant = this.getVariantJSON().find((variant) => {
        // Adjust the property name based on your actual data structure
        return variant.option1 === singleOption || variant.option2 === singleOption || variant.option3 === singleOption;
      });
    }
  ...
  updateFormID() {
    const form_input = document.querySelectorAll(
      `#product-form-${this.dataset.section}, #product-form-installment-${this.dataset.section}`
    );
    form_input.forEach((productForm) => {
      const input = productForm.querySelector('input[name="id"]');
      input.value = this.currentVariant.id;
      input.dispatchEvent(new Event('change', { bubbles: true }));
    });
